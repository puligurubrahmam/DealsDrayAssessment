...
const UpdateEmployer =(props)=>
{
    ...
    const submitForm = async (event)=>
    {
        ...
        const option ={
            method:'PUT',
            body:JSON.stringify({
                name,email,mobile,designation,gender,course,image,user:loginned,emp
            }),
            headers:{
                'Content-Type':'application/json'
            }
        }
        const response =await fetch("http://localhost:5000/updateemp",option);
        ...
        return (
            ...
            <div>
                ...
                <form className='form-item' onSubmit={submitForm}>
                    ...
                    <button className='create-btn' onClick={submitForm}>Update</button>
                </form>
                </div>
            </div>
        )
